...
func emailResponse(db *sql.DB, email string) (*pb.EmailResponse, error) {
	mdbEntry, err := mdb.GetEmail(db, email)
	if err != nil {
	...
	res := mdbEntryToPbEntry(mdbEntry)
...
func Serve(db *sql.DB, bind string) {
	...
	if err != nil {
		log.Fatalf("gRPC server error: failed to listen: %v", err)
	}
	...
	if err := grpcServer.Serve(listener); err != nil {
		log.Fatalf("gRPC server error: failed to serve: %v", err)
	}
}
